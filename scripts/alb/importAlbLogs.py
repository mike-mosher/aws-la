...
def createAlbIndexAndMapping():
    # Mapping Name: options.index_name
    print "Creating mapping in ES for index: %s" % (options.index_name)

    #open mappings file
    with open('alb-mapping.json') as f:
        alb_mapping = json.load(f)

    es.indices.create(index=options.index_name, ignore=400, body=alb_mapping['alb_logs'])

def putIngestPipeline():
    print 'Creating Ingest Pipeline for index: ' + options.index_name

    #open mappings file
    with open('alb-ingestPipeline.json') as f:
...
def updateKibanaIndexMapping():
    ...
    with open('alb-kibana-index-mapping.json') as f:
...
def createKibanaIndexAlbIndexPattern():
    ...
    with open('alb-kibana-index-data.json') as f:
        data = json.load(f)

        for i in data['hits']['hits']:
            if i['_type'] == 'index-pattern' and i['_id'] == 'alb_logs':

                url = 'http://' + options.es_host + ':5601/elasticsearch/.kibana/index-pattern/alb_logs/_update'
                headers = { 'kbn-version': '5.4.0' }
    ...
    with open('alb-kibana-index-data.json') as f:
        ...
        for i in data['hits']['hits']:
            ...
            if i['_type'] == 'dashboard':
                ...
                albDashboardId = i['_id']

    return albDashboardId
    ...
    for root, dirs, files in os.walk(options.log_directory):
        for log_file in files:
            if log_file.endswith(".gz"):
                ...
                print "File: " + log_file + " is not an ALB access log file. Log files need to end with *.gz"
...
parser = optparse.OptionParser(
                    usage="Send ALB access logs to ES",
                    version="0.1"
                  )
...
parser.add_option('-i',
                  ...
                  dest="index_name",
                  default="alb_logs",
                  help="name of the index to create if not default of alb_logs"
                  )
...
createKibanaIndexAlbIndexPattern()
...
albDashboardId = importObjectsToKibana()
...
if albDashboardId:
    # If albDashboardId has been set, then send them directly to the dashboard URL
    url = 'http://' + options.es_host + ':5601/app/kibana#/dashboard/' + albDashboardId + url_timeframe
else:
