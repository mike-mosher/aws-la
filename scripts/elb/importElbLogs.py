...
def updateKibanaIndexMapping():
    ...
    with open('elb-kibana-index-mapping.json') as f:
...
def createKibanaIndexElbIndexPattern():
    ...
    with open('elb-kibana-index-data.json') as f:
        data = json.load(f)
...
parser = optparse.OptionParser(
                    usage="Send ELB access logs to ES",
                    version="0.1"
                  )
...
parser.add_option('-i',
                  ...
                  dest="index_name",
                  default="elb_logs",
                  help="name of the index to create if not default of elb_logs"
                  )
