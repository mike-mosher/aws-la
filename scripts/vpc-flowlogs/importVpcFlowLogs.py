...
def createVflIndexAndMapping():
    # Mapping Name: options.index_name
    print "Creating mapping in ES for index: %s" % (options.index_name)

    #open mappings file
    with open('vfl-mapping.json') as f:
        vfl_mapping = json.load(f)

    es.indices.create(index=options.index_name, ignore=400, body=vfl_mapping['vpc_flowlogs'])

def putIngestPipeline():
    print 'Creating Ingest Pipeline for index: ' + options.index_name

    #open mappings file
    with open('vfl-ingestPipeline.json') as f:
...
def updateKibanaIndexMapping():
    ...
    with open('vfl-kibana-index-mapping.json') as f:
...
def createKibanaIndexVflIndexPattern():
    ...
    with open('vfl-kibana-index-data.json') as f:
        data = json.load(f)

        for i in data['hits']['hits']:
            if i['_type'] == 'index-pattern' and i['_id'] == 'vpc_flowlogs':

                url = 'http://' + options.es_host + ':5601/elasticsearch/.kibana/index-pattern/vpc_flowlogs/_update'
                headers = { 'kbn-version': '5.4.0' }
    ...
    with open('vfl-kibana-index-data.json') as f:
        ...
        for i in data['hits']['hits']:
            ...
            if i['_type'] == 'dashboard':
                ...
                vflDashboardId = i['_id']

    return vflDashboardId
    ...
    for root, dirs, files in os.walk(options.log_directory):
        for log_file in files:
            if log_file.endswith(".gz"):
                ...
                print "File: " + log_file + " is not an VPC flowlog file. Log files need to end with *.gz"
...
parser = optparse.OptionParser(
                    usage="Send VPC flow logs to ES",
                    version="0.1"
                  )
...
parser.add_option('-i',
                  ...
                  help="name of the index to create if not default of vpc_flowlogs"
                  )
...
# Create a new index-pattern in .kibana index for alb_logs
createKibanaIndexVflIndexPattern()
...
vflDashboardId = importObjectsToKibana()
...
if vflDashboardId:
    # If vflDashboardId has been set, then send them directly to the dashboard URL
    url = 'http://' + options.es_host + ':5601/app/kibana#/dashboard/' + vflDashboardId + url_timeframe
else:
